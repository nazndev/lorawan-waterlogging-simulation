...
with theory_tabs[0]:
    ...
    **Current Simulation Parameters:**
    ...
    path_losses = []
    for d in distances:
        pl = WIRELESS_CONFIG["PL0"] + 10 * WIRELESS_CONFIG["path_loss_exponent"] * np.log10(d / WIRELESS_CONFIG["d0"])
...
with theory_tabs[1]:
    ...
    **Relationship to SNR:**
    ...
    for snr in snr_range:
        # Simplified PER model for visualization
...
with theory_tabs[4]:
    st.markdown("### Real-World Applications & Course Relevance")
    
    st.markdown("""
    #### Implementation Summary
    
    **1. Path Loss Modeling** (Propagation Models)
    - Log-distance path loss model: PL(d) = PL₀ + 10·n·log₁₀(d/d₀) + X_σ
    - Shadowing: Log-normal distribution (σ = 8.0 dB)
    - Path loss exponent: n = 3.5 (urban environment)
    - Application: Signal strength prediction for Dhaka city
    
    **2. SNR & Link Quality** (Signal Quality Metrics)
    ...
    **4. Network Capacity** (System Design)
    ...
    #### 5. **IoT & LPWAN** (Course Topic: Emerging Technologies)
    ...
    **Course Learning Outcomes:**
    1. Wireless propagation models
    2. SNR and link quality metrics
    3. Multiple access schemes
    4. Network capacity and trade-offs
...
st.markdown("### 📊 Network Configuration: Spreading Factor Distribution")
devices = get_all_devices(db)
sf_counts = {}
for device in devices:
    sf = device.spreading_factor
...
if sf_counts:
    ...
    df_sf = df_sf.sort_values("Spreading Factor")
    
    fig_sf = px.pie(
        df_sf,
        values="Device Count",
        names="Spreading Factor",
        title="Device Distribution by Spreading Factor",
    ...
    **Network Design Analysis:**
    - Distribution across SF values reduces collision probability
    - Devices automatically select SF based on link quality (distance to gateway)
    - This creates a **self-organizing network** that adapts to channel conditions
    """)
...
2. Signal Quality: SNR, RSSI, and relationship to PER
3. Modulation: Chirp Spread Spectrum (CSS) and Spreading Factor trade-offs
4. Multiple Access: Orthogonal channels, collision detection, duty cycle
5. Network Design: Capacity analysis, range vs. throughput trade-offs
6. Application: IoT sensor network for smart city monitoring

**System Implementation:**
- Mathematical models: Path loss, SNR, PER calculations
- Implementation: Python-based simulation with PostgreSQL storage
