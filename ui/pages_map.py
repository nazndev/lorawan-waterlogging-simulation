...
def render():
    ...
    try:
        ...
        if "pydeck_warning_shown" not in st.session_state:
            st.session_state.pydeck_warning_shown = True
            st.info("ℹ️ Map visualization uses table view. Install pydeck for interactive maps: `pip install pydeck`")
    ...
    for item in devices_with_readings:
        ...
        # Initial color assignment (will be overridden by view mode)
        # Default: color by water level, but consider device status
        if device.status == DeviceStatus.OFFLINE:
            color = [255, 0, 0, 200]  # Red for offline devices
        elif device.id in alert_device_ids:
    ...
    if map_data:
        ...
        if view_mode == "Water Level":
            ...
            for i, row in df_map.iterrows():
                ...
                if device and device.id in alert_device_ids:
                    ...
                    df_map.at[i, "color"] = [0, 255, 0, 200]  # Green
        elif view_mode == "Device Status":
            # Color by device status: Online = Green, Offline = Red, Maintenance = Yellow
            for i, row in df_map.iterrows():
                device_id_str = row["device_id"]
                device = next((d for d in get_all_devices(db) if d.device_id == device_id_str), None)
                if device:
                    if device.status == DeviceStatus.ONLINE:
                        df_map.at[i, "color"] = [0, 255, 0, 200]  # Green for online
                    elif device.status == DeviceStatus.OFFLINE:
                        df_map.at[i, "color"] = [255, 0, 0, 200]  # Red for offline
                    elif device.status == DeviceStatus.MAINTENANCE:
                        df_map.at[i, "color"] = [255, 255, 0, 200]  # Yellow for maintenance
                    else:
                        df_map.at[i, "color"] = [128, 128, 128, 200]  # Gray for unknown
                else:
                    df_map.at[i, "color"] = [128, 128, 128, 200]  # Gray if device not found
        ...
        if PYDECK_AVAILABLE:
            ...
            deck = pdk.Deck(
                map_style="road",  # Use basic road style (no Mapbox token required)
        ...
        with col3:
            ...
            st.metric("Devices on Map", len(df_map))
            st.metric("Devices with Readings", devices_with_data)
            st.metric("Devices with Alerts", len(alert_device_ids))
        
        # Note: Detailed device table is available on the Devices page to avoid redundancy
    else:
